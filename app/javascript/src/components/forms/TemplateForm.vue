<template>
  <div class="mx-auto inline w-full">
    ...
    >
      <ul class="grid w-full gap-6 grid-cols-2">
        <li v-for="item in allThemes">
          <InputField
            :inputId="item"
            :className="'my-3 inline'"
            :inputValue="item"
            :inputFieldClass="'hidden peer'"
            :inputName="'theme'"
            :inputType="'radio'"
            :checked="item === selectedTheme"
            @changeValue="themeStore.onToggleTheme(item)"
          >
            <template #label>
              <label
                :for="item"
    ...
    >
      <LabelWrapper :forAttribute="'per-page'" :textClass="''" :labelText="''">
        <ul class="grid w-full gap-6 grid-cols-2">
          <li v-for="item in backgroundImages">
            <InputField
              :inputId="item.className"
              :className="'my-3 inline'"
              :inputValue="item.className"
              :inputFieldClass="'hidden peer'"
              :inputName="'backgroundImages'"
              :inputType="'radio'"
              :checked="item.className === selectedImages"
              @changeValue="themeStore.changeBackgroundImage"
            >
              <template #label>
                <label
                  :for="item.className"
