<template>
  <div class="min-h-screen flex justify-center">
    <div v-if="isLoading === false" class="mt-16 w-full mx-2">
      ...
      <div
        class="grid grid-cols-1 sm:grid-cols-2 gap-2 md:flex md:justify-center"
      >
        <div
          v-for="(num, key) in totalNumbers"
          class="bg-dark p-6 rounded-xl m-1 w-full md:w-64"
        >
          <span class="block text-info text-lg">
            <font-awesome-icon :icon="`fa-solid fa-${icon(key)}`" />
            {{ key.toUpperCase() }} :
          </span>
          ...
          <div class="block mt-3">
            <router-link
              class="block text-light bg-warning w-fit rounded p-2 float-left"
              :to="`/${key}/add`"
            >
              Add +
            </router-link>
            <router-link
              class="block text-light bg-success w-fit rounded p-2 float-right"
              :to="`/${key}/list`"
            >
              More →
            </router-link>
